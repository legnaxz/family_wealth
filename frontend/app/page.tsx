      ...
      <section style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
        ...
        <div style={card}>
          ...
          <div style={{ display: 'flex', gap: 10, marginBottom: 8, fontSize: 13 }}>
            <span>수입 <b>{won(flow?.summary?.income || 0)}</b></span>
            <span>지출 <b>{won(flow?.summary?.expense || 0)}</b></span>
        ...
        <div style={card}>
          ...
          {(bs.assets || []).slice(0, 6).map((x: any, i: number) => (
            <div key={i} style={{ marginBottom: 8 }}>
              ...
              <div style={{ height: 7, background: '#dbeafe', borderRadius: 999 }}><div style={{ width: `${x.weight}%`, height: 7, background: '#2563eb', borderRadius: 999 }} /></div>
            </div>
          ))}
          <div style={{ margin: '8px 0' }}>부채 총액 <b>{won(bs.liabilitiesTotal || 0)}</b></div>
          {(bs.liabilities || []).slice(0, 6).map((x: any, i: number) => (
            <div key={i} style={{ marginBottom: 8 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13 }}><span>{x.name}</span><b>{x.weight}%</b></div>
              <div style={{ height: 7, background: '#f3e8ff', borderRadius: 999 }}><div style={{ width: `${x.weight}%`, height: 7, background: '#a21caf', borderRadius: 999 }} /></div>
            </div>
          ))}
        </div>
        ...
        <div style={card}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            <div>
              <div style={{ fontSize: 12, color: '#991b1b', marginBottom: 6 }}>지출</div>
              {expenseShare.slice(0, 8).map((c: any, i: number) => (
                <div key={i} style={{ marginBottom: 8 }}>
                  ...
                  <div style={{ height: 7, background: '#fee2e2', borderRadius: 999 }}><div style={{ width: `${c.weight}%`, height: 7, background: '#ef4444', borderRadius: 999 }} /></div>
                </div>
              ))}
            </div>
            <div>
              <div style={{ fontSize: 12, color: '#166534', marginBottom: 6 }}>수입</div>
              {incomeShare.slice(0, 8).map((c: any, i: number) => (
                <div key={i} style={{ marginBottom: 8 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12 }}><span>{c.category}</span><b>{c.weight}%</b></div>
                  <div style={{ height: 7, background: '#dcfce7', borderRadius: 999 }}><div style={{ width: `${c.weight}%`, height: 7, background: '#22c55e', borderRadius: 999 }} /></div>
                </div>
              ))}
            </div>
          </div>
